# THE RULES OF DUTCH NUMBERS
#
# - Implement a number system (natural numbers, integers, rationals, reals, etc)
#   in your programming language of choice
#
# - Your number system cannot be internally represented by any of the
#   numeric datatypes in your implementation language
#
# - Points for how fully-featured your number system is
#
# - Points for how mathematically opulent your number system is
#
# - Points for how ridiculous your number system is
#
# - No points for speed or numerical efficiency


# Counter
# An object with magnitude used to represent natural numbers
class Counter
  include Comparable

  def initialize(value = nil)
    @magnitude = nil

    if value.is_a?(Array) && value.flatten.shift.nil?
      @magnitude = value
    end

    # We are totally allowed to use numerics when interpolating numerics
    if value.is_a? Integer
      (1..(value.abs)).each do
        @magnitude = increment(@magnitude)
      end
    end
  end

  # Deep copy of the object's magnitude
  def mag
    Marshal::load(Marshal.dump(@magnitude))
  end

  # Helper method for finding the successor of a numeric representation
  def increment(inner = nil)
    [inner]
  end

  ## Helper method for finding a numeric representation's predecessor
  def decrement(outer)
    outer.shift
  end

  # this ++
  def inc
    @magnitude = increment(@magnitude)
    self
  end

  # this --
  def dec
    @magnitude = decrement(@magnitude)
    self
  end

  # Helper method for getting the additive identity
  def zero
    Counter.new(nil)
  end

  # Helper method for getting the multiplicative identity
  def one
    Counter.new([nil])
  end

  def zero?
    @magnitude.nil?
  end

  # kind of like .foreach, but counting down over the value
  def over
    mag = self.mag

    loop do
      break if mag.nil?
      mag = decrement(mag)
      yield(mag)
    end
  end

  def inspect
    to_s
  end

  def to_s

    n = []
    over { n.push([]) }
    n.length.to_s

  end

  def +(other)
    b_mag = other.mag
    over {
      b_mag = increment(b_mag)
    }

    Counter.new(b_mag)

  end

  def *(other)
    cardinality = nil
    over do
      other.over do
        cardinality = increment(cardinality)
      end
    end

    Counter.new(cardinality)

  end

  def <=>(other)
    max = self + other

    # Thank you Ruby for allowing deep nested array comparisons
    return 0 if mag == other.mag

    # Iterate over both values;
    # Whichever one we hit first is the lowest
    max.over do |i|
      return 1 if i == mag
      return -1 if i == other.mag

      # Should never get here
      raise RangeError if i == max.mag
    end

  end
end


# Dutch Integers
# Values with a positive and negative natural number component
class Dint
  include Comparable

  attr_accessor :positive, :negative
  def initialize(value = nil, positive = true)
    @positive = nil
    @negative = nil

    magnitude = Counter.new(value)
    # If the value is negative or the sign boolean suggests so...
    if (value.is_a?(Integer) && value.negative?) || !positive
      @negative = magnitude
      @positive = Counter.new
    else
      @positive = magnitude
      @negative = Counter.new
    end

    if value.is_a? Dint
      @positive = Counter.new(value.positive.mag)
      @negative = Counter.new(value.negative.mag)
    end
  end

  # Cancel down our positive and negative components until
  # one of them is zero
  def reduce_to_canonical
    while !@positive.zero? && !@negative.zero?
      @positive.dec
      @negative.dec
    end

  end

  def inspect
    to_s
  end

  def to_s
    reduce_to_canonical
    neg? ? ('-' << @negative.to_s) : @positive.to_s
  end

  def pos?
    reduce_to_canonical
    @negative.zero?
  end

  def neg?
    reduce_to_canonical
    !@negative.zero?
  end

  def <=>(other)
    diff = self - other
    diff.reduce_to_canonical

    # Note to readers: the comparators below
    # are on `Counter` objects, not on any of
    # Ruby's numeric datatypes
    return 0 if diff.positive == diff.negative
    return -1 if diff.positive > diff.negative
    return 1 if diff.positive < diff.negative

    # nil if this somehow fails
    nil
  end

  # Unary `-`: just swap the components
  def -@
    @positive, @negative = @negative, @positive
  end

  # Addition: just sum the components
  def +(other)
    ret = Dint.new
    ret.positive = @positive + other.positive
    ret.negative = @negative + other.negative
    ret
  end

  # Subtraction: just sum opposite components
  def -(other)
    ret = Dint.new
    ret.positive = @positive + other.negative
    ret.negative = @negative + other.positive
    ret
  end

  # Multiplication: positive component is sum of cis-products
  # negative component is sum of trans-products
  def *(other)
    ret = Dint.new
    ret.positive = (@positive * other.positive) + (@negative * other.negative)
    ret.negative = (@negative * other.positive) + (@positive * other.negative)
    ret
  end

  # Integer division
  def /(other)
    ret = Dint.new

    p_count = Counter.new
    n_count = Counter.new

    # Over the positive component, decrement `self` every `other`th iteration
    @positive.over do
      p_count.inc
      n_count.inc
      ret.positive.inc if p_count == other.positive
      ret.negative.inc if n_count == other.negative
      p_count = p_count.zero if p_count == other.positive
      n_count = n_count.zero if n_count == other.negative
    end

    p_count = p_count.zero
    n_count = n_count.zero

    # Over the negative component, decrement `self` every `other`th component
    @negative.over do
      n_count.inc
      p_count.inc
      ret.negative.inc if n_count == other.positive
      ret.positive.inc if p_count == other.negative
      p_count = p_count.zero if p_count == other.negative
      n_count = n_count.zero if n_count == other.positive
    end

    ret
  end

end



